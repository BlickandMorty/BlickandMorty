...
* **[Representation–Causality Gap Audit](https://github.com/BlickandMorty/representation-causality-gap-audit)** — A layer-20 ridge probe scored `100%` under leave-one-entity-out development CV, then collapsed to `50%` on 120 new science-format cases: `0%` on REFUTED and CONFLICT, `100%` on SUPPORTED and UNKNOWN. The split blocked names but not template leakage, so I reject general four-state decodability.
