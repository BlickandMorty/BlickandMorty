...
* **[Scientific Evidence-State Transport](https://github.com/BlickandMorty/scientific-evidence-state-transport)** — A frozen 36-pair chemistry/physics/biology transport test of that layer-20 intervention. The causal sign held on `31/36` pairs in both directions and a norm-matched random control centered near zero, but the mean forward effect fell to `+2.349`—about `17.6%` of the earlier magnitude—and physics was positive on only `8/12` pairs.
