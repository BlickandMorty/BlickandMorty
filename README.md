...
* **[Scientific Reasoning Audit Loops](https://github.com/BlickandMorty/scientific-reasoning-audit-loops)** — Qwen3 1.7B on controlled chemistry/physics/biology calculations. The original 72-case test found self-audit `61.1%→79.2%` but three regressions; verifier-gated repair reached `76.4%` with zero regressions by design. A public-precommitted, exactly A/B/C/D-balanced 72-case confirmation replicated both effects: direct `62.5%`, intrinsic `75.0%` (+12.5, CI +2.8 to +23.6), verifier-gated `81.9%` (+19.4, CI +11.1 to +29.2; 14 repairs, 0 regressions).
* **[Dose-Response Audit Lab](https://github.com/BlickandMorty/dose-response-audit-lab)** — Reproducible 4PL/IC50 Monte Carlo studies separating outlier robustness from concentration-window identifiability. Missing a plateau made roughly 69–74% of potency fits at least twofold wrong even when the optimizer reported success. A separately preregistered 400-curve-per-condition control-bias sweep found a monotonic recovery-risk curve: at 40 response units of fixed-control bias, about 54% of fits were at least twofold wrong. The SHA-256 receipt verifies replay identity, not biological truth.
* **[Proof-Carrying Policy Evals](https://github.com/BlickandMorty/proof-carrying-policy-evals)** — A defensive ALLOW/DENY/ESCALATE benchmark with a deterministic oracle, SHA-256 receipts, and 96 metamorphic cases that distinguish stable reasoning from being consistently wrong. A later public-precommitted 72-case certificate-format confirmation increased Qwen3 accuracy 40.3%→59.7% (paired CI +11.1 to +29.2) but raised unauthorized ALLOWs 3→6, so it failed the safety rule: aggregate accuracy is not a safety result.
